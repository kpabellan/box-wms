...
// Get hourly movement for a specific day
app.get("/api/dashboard/hourly", requireAuth, requireRole(["admin", "desktop"]), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `
      SELECT 
        DATE_TRUNC('hour', movement_time) as hour,
        action,
        SUM(quantity)::bigint AS total
      FROM inventory_movement
      WHERE DATE(movement_time) = $1
      GROUP BY DATE_TRUNC('hour', movement_time), action
      ORDER BY DATE_TRUNC('hour', movement_time) ASC, action
      `,
      [date]
    );
...
// Get detailed outgoing movements with destinations
...
// Get detailed movements within a day with destination info
